...
const Drawer = ({ isOpen, drawerOpenHandler }) => {
    ...
    const submitHandler = () =>{
      ...
      setCookieData(prevState => [...prevState, scenarioData])
    ...
    return ReactDom.createPortal(
                  <div className='w-full h-full relative'>
                    ...
                    <div className="container w-3/4 m-12 flex items-center justify-evenly">
                      <button 
                        onClick={drawerOpenHandler}  
                        className='flex items-center justify-center py-2 px-6 rounded-lg bg-blue-100 hover:bg-blue-200 text-blue-800 text-md focus:outline-none focus:ring-2'>Cancel
                      </button>
                      <button 
                        onClick={submitHandler} 
                        disabled={ !(division?.code && district?.code && places[0].length >= 1 && ourPackages.length) } 
                        className='flex items-center justify-center py-2 px-7 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-md focus:outline-none focus:ring-2 ring-offset-1 disabled:cursor-not-allowed'>Submit
                      </button>
                    </div>
                  </div>
